 ...
 */
 function drawWaves(ordered_color_array, waves_height, canvas_dim, max_colors_waves){
  ...
  if(ordered_color_array.length>max_colors_waves)
      wave_colors =  p5js.subset(ordered_color_array, (ordered_color_array.length-1)-max_colors_waves, (ordered_color_array.length));
  else 
      wave_colors =  ordered_color_array;
    
  const stripe_width = (canvas_dim.h-waves_height)/(wave_colors.length-1);
  for(let i = (wave_colors.length-1); i>0; i--){
    p5js.fill(wave_colors[i]);
    p5js.noStroke();  
    
    //create array of vertexes
    const this_curvey_line_y_pnt  = (waves_height)+((wave_colors.length-1-i)*stripe_width);
    const num_points_increment = 5;
    const percentage_variation = 60
    const arrayOfPointsForCurveyLine = createArrayOfPointsForCurveyLine(1, percentage_variation, stripe_width, ((wave_colors.length-i)*num_points_increment), this_curvey_line_y_pnt,canvas_dim);
    ...
    for(let vertex_counter =0; vertex_counter<arrayOfPointsForCurveyLine.length; vertex_counter++ ){         
      p5js.curveVertex(arrayOfPointsForCurveyLine[vertex_counter].x,arrayOfPointsForCurveyLine[vertex_counter].y);
    }
    p5js.endShape(CLOSE);
    ...
    for(let shade_counter = 0; shade_counter<(stripe_width*2); shade_counter+=shade_spacing){
      ...
      p5js.fill(getDarkerOrLighterVersionOfColor(wave_colors[i], 1, (shade_counter*shade_increment)));
      p5js.beginShape();
      for(let vertex_counter =0; vertex_counter<arrayOfPointsForCurveyLine.length; vertex_counter++ ){                
      p5js.curveVertex(arrayOfPointsForCurveyLine[vertex_counter].x,
 ...
 */
 function drawMoons(num_moons, ordered_color_array, sky_height, sky_width, ascending_size_flag){
   let this_moon_color;
   for(let moon_counter = 0; moon_counter<num_moons; moon_counter++){
      ...
      case 2:
        moon_color_index = 1;
         this_moon_color= getDarkerOrLighterVersionOfColor(ordered_color_array[moon_color_index], coinFlip(), p5js.random(75));
        break;
      case 3:
        moon_color_index = 1+moon_counter;
        this_moon_color= getDarkerOrLighterVersionOfColor(ordered_color_array[moon_color_index], coinFlip(), p5js.random(50));
        break;
      default:
        moon_color_index =1+p5js.round(p5js.random(ordered_color_array.length-3));
    ...
    if(ascending_size_flag){
      moon_diameter= p5js.random(((150*sky_width)/1500)+(((60*sky_width)/1500)*moon_counter),((200*sky_width)/1500)+(((60*sky_width)/1500)*moon_counter));
    }else{
      moon_diameter= p5js.random(((150*sky_width)/1500)-(((60*sky_width)/1500)*moon_counter),((200*sky_width)/1500)-(((60*sky_width)/1500)*moon_counter));
    }   
    let moon_y_pnt = p5js.random(75,(sky_height-75));
   
    //spread them out horizontally
    let direction = coinFlip();
    let moon_x_pnt = ((sky_width/num_moons)*moon_counter)+((sky_width/num_moons)/2)+(direction*p5js.random((moon_diameter/4)));  
    //now draw darkening lines in that color, to give the shape shading
    const shade_spacing = 1;
    const shade_increment = .20;
    const number_of_shade_iterations = ((moon_diameter)/2);
    //get a darker version of the target color
    const starting_color = getDarkerOrLighterVersionOfColor(this_moon_color, 1, (number_of_shade_iterations*shade_increment)*2);
    
    //const stripe_width
    for(let shade_counter = 0; shade_counter<number_of_shade_iterations; shade_counter+=shade_spacing){
       p5js.fill(getDarkerOrLighterVersionOfColor(this_moon_color, 0, (shade_counter*shade_increment)));
       p5js.ellipse(moon_x_pnt, moon_y_pnt-(shade_counter/4), moon_diameter-(shade_counter/4), moon_diameter-(shade_counter));
    }
  }
 }
 ...
 */
 function drawStars(num_stars, ordered_color_array, sky_height, sky_width){
   ...
   const num_cols = p5js.round(num_stars/4);
   const num_rows = p5js.round(num_stars/num_cols)+5;
   ...
   for(let col_counter = 0; col_counter<num_cols; col_counter++){
    for(let row_counter = 0; row_counter<num_rows; row_counter++){
      ...
      case 2:
        ...
        this_star_color= getDarkerOrLighterVersionOfColor(ordered_color_array[random_color_index_not_darkest], coinFlip(), p5js.random(50));
        break;
      case 3:
        if(coinFlip()==1)
          random_color_index_not_darkest = 1;
        else
          random_color_index_not_darkest = 2;
        this_star_color= getDarkerOrLighterVersionOfColor(ordered_color_array[random_color_index_not_darkest], coinFlip(), p5js.random(50));
        break;
      default:
        random_color_index_not_darkest = p5js.round(p5js.random(1,(ordered_color_array.length-1)));
        this_star_color=ordered_color_array[random_color_index_not_darkest];
        break;
    }
      
      
     let star_y_pnt = (row_counter*row_spacing)+(p5js.random(row_spacing/2)*coinFlip());
     //spread them out horizontally
     let star_x_pnt =  (col_counter*col_spacing)+(p5js.random(col_spacing/2)*coinFlip());  
     //set diameter and account for different sized canvases
     let star_diameter= p5js.floor(p5js.random(((2*sky_width)/1500),((10*sky_width)/1500)));
     p5js.fill(this_star_color);
     p5js.ellipse(star_x_pnt, star_y_pnt, star_diameter, star_diameter);
   }
  }    
}
...
function coinFlip(){
  ...
  p5js.background(orderedColorArray[0]);
  p5js.noLoop();
