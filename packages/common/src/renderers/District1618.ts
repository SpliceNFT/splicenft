 ...
 */
function layoutGridOnPartOfCanvas(goldenMeanRectArray, grid_dim, start_pnt, colors){  
  const total_rows = p5.floor(grid_dim.h/goldenMeanRectArray[0].sqr_width);
  const length_of_largest_width = (goldenMeanRectArray[0].sqr_width+goldenMeanRectArray[0].rect_width);
  const total_cols = p5.round(grid_dim.w/length_of_largest_width);
  const horiz_margin = (grid_dim.w-(total_cols*length_of_largest_width))/2;
  const vert_margin = (grid_dim.h-(total_rows*goldenMeanRectArray[0].sqr_width))/2;
  ...
  for(let row = 0; row<total_rows; row++){
    for(let col = 0; col<total_cols; col++){
      center_point = new PointLocation(horiz_margin+(col*length_of_largest_width)+(length_of_largest_width/2),
      vert_margin+(row*goldenMeanRectArray[0].sqr_width)+(goldenMeanRectArray[0].sqr_width/2) );
...
function drawDrosteEffect(goldenMeanRectArray, center_point, colors){  
  for(let i=0; i<goldenMeanRectArray.length; i++){
    let a_GoldenMeanRect = goldenMeanRectArray[i];
    //draw it
    p5.push();
    let width_of_sqr_and_rect = a_GoldenMeanRect.sqr_width+a_GoldenMeanRect.rect_width;
    p5.translate(center_point.x-(width_of_sqr_and_rect/2), center_point.y-(a_GoldenMeanRect.sqr_width/2));
...
class GoldenMeanRect {
  constructor(x_, y_, length_) {
    ...
    this.rect_width =  (p5.sqrt(p5.sq(length_)+sq((length_/2))))-(length_/2);
    ...
    this.rectVertexArray.push(new PointLocation(x_+length_+this.rect_width, y_+length_));    
    this.rectVertexArray.push(new PointLocation(x_+length_+this.rect_width, y_));
  }
  ...
  renderRect(){
    ...
    for(let i = 0; i<this.rectVertexArray.length; i++){
      p5.vertex(this.rectVertexArray[i].getX(), this.rectVertexArray[i].getY());
    }
    p5.endShape(CLOSE); 
  }
...
function getColor(colors, colorIndex){
  switch (colorIndex) {
    ...
    case 2:
      ...
      let r = floor(p5.random(num_tertiary_colors));
      return(colors[1+r]);
      break;
    case 3:
 ...
 */
  let arrayOfGoldenMeanRectArray = [];
  let center_point;
  let grid_dim;
  let start_pnt;
  ...
  for(let grid_counter = 0;grid_counter<num_cols_across_grid_array.length;grid_counter++  ){
    let rect_and_square_width = dim.w/num_cols_across_grid_array[grid_counter];
    //let rect_and_square_width = dim.w/num_cols_across_grid;
    let sqr_width = rect_and_square_width/1.618;
    let goldenMeanRectArray = [];
    //create an array of shrinking goldenMeanRects
    for(let i = 0; i<total_shrinking_sqrs; i++){
      let a_GoldenMeanRect =  new GoldenMeanRect(
        0, 0, sqr_width
      ); 
  ...
  for(let grid_index = 0;grid_index<customized_order_of_drawing.length;grid_index++  )   {
    let grid_counter = customized_order_of_drawing[grid_index];
    grid_dim = new Dimension(dim.w/arrayOfGoldenMeanRectArray.length, dim.h);
    start_pnt = new PointLocation((grid_counter*(dim.w/arrayOfGoldenMeanRectArray.length)),0);
    layoutGridOnPartOfCanvas(arrayOfGoldenMeanRectArray[grid_counter], grid_dim, start_pnt, colors);
  }
}
