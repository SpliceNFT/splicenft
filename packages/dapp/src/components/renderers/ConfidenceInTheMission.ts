...
export default function ({ p5, colors, dim }: DrawProps) {
  ...
  p5.smooth();
  let stroke_size = p5.floor(dim.w/187);
  ...
  const additional_horizontal_columns = (((num_rows/2)*(max_horiz_shift))/(quad_width+horiz_space))*2;
  ...
  const height_of_flat = (num_of_flat_grid_middle_horizontal_center_rows*2*(quad_height+vert_space));
  const flat_quad_height =p5.round(dim.h/5.86);
  const flat_num_columns = p5.floor(dim.w/(flat_quad_width+flat_horiz_space));
  const flat_num_rows = p5.floor(height_of_flat/(flat_quad_height+flat_vert_space) );
  const flat_horiz_margin = (height_of_flat-(flat_num_rows*(flat_quad_height+flat_vert_space)))/2;
  ...
  for (let row_counter = 0; row_counter < flat_num_rows; row_counter++) {  
    ...
    p5.translate(
      0,(flat_horiz_margin+((num_rows/2)-num_of_flat_grid_middle_horizontal_center_rows)*(quad_height+vert_space))+row_counter*(flat_quad_height+flat_vert_space)
    );
    ...
    for (let col_counter = 0; col_counter <flat_num_columns; col_counter++) {   
      ...
      if((col_counter%flat_frequency_of_background_color_col)==0){
        ...
        if(p5.floor(random(4))>0){
          flat_background_color_num_rows = int(p5.random(1,(flat_frequency_of_background_color_col-2)));
         }
        //get a tertiary color for that row 
        flat_background_color_num_rows_color = getColorForFlatGridRect(colors,2);
      }
  ...
  for (let col_counter = -1*additional_horizontal_columns; col_counter <(num_columns+additional_horizontal_columns); col_counter++) {
    p5.push()
    ...
    if((col_counter<((num_columns/2)+num_dark_cols_in_center_row))&&
      ...
      this_color.setAlpha(p5.getAlpha(row_counter/num_rows));//STEFAN - SHOULD WE ADD P5. BEFORE this_color.setAlpha?
    }
         ...
         p5.quad(0,0, 
                    shift_dir*(((num_columns/2)-col_counter)*horiz_shift_increment),quad_height, 
                     shift_dir*(((num_columns/2)-col_counter)*horiz_shift_increment)+quad_width, quad_height, 
                      quad_width, 0);
       p5.translate(shift_dir*(((num_columns/2)-col_counter)*horiz_shift_increment), (quad_height+vert_space));
     }else{ 
        //draw the rect already!  
        p5.quad(0,0, shift_dir*(-((col_counter-(num_columns/2))*horiz_shift_increment)),quad_height, 
            shift_dir*(-((col_counter-(num_columns/2))*horiz_shift_increment))+quad_width, quad_height, 
            quad_width, 0); 
              
        //move to the next rect
        p5.translate(shift_dir*(-((col_counter-(num_columns/2))*horiz_shift_increment)), (quad_height+vert_space));
     }
    }
    p5.pop();
  }
...
*/
function getColorForFlatGridRect(colors, colorIndex){
    let a_new_color;
  ...
  switch (colorIndex) {
    ...
    case 1:
      ...
      if((red(colors[1])==0)&&(green(colors[1])==0)&&(blue(colors[1])==0))
...
*/
function getAlpha(grid_index){
  let alpha;
  let max_alpha = 255;
  let min_alpha = 50;
  let range = max_alpha-min_alpha;
  if(grid_index<.5){
