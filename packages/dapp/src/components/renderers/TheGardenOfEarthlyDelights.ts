...
/**
 * TheGardenOfEarthlyDelights generates a mildly post-apocolyptic landscape,   
...
function plantFractal(colors, dim, max_seeds, fractalObjects){
  let x_point;
  const wireSeedNum = int(p5.random(1,8));
  const towerSeedNum = int(p5.random(8,10));
  ...
  let halfway_max_seeds = max_seeds/2;
  ...
  if(fractalObjects.length<halfway_max_seeds){
     ...
     this_index = halfway_max_seeds-fractalObjects.length;
      ...
      this_index = halfway_max_seeds-(fractalObjects.length-halfway_max_seeds);
      x_point = (this_index)*(((dim.width)/2)/halfway_max_seeds);
  }

  //seed with more wires than towers
  if(int(p5.random(100)<75))
    seed(wireSeedNum, x_point,this_index,halfway_max_seeds, colors, dim, fractalObjects);
  else
    seed(towerSeedNum, x_point,this_index,halfway_max_seeds, colors, dim, fractalObjects);
}
...
function getLikeColor(a_color){
  ...
  let new_total_colors = int(p5.random(0,(255*3)));
 ...
 */
function seed(rule_index, y_point, seed_index, max_seeds, colors, dim, fractalObjects){
  let a_rule_object;
  let lsys;
  let ruleset = [];
  ...
  if (rule_index==0)
    ...
    switch (rule_index) {
      ...
      case 1:
        ...
        num_radians = int(p5.random(80,150));
        if(p5.random(100)>50)
          num_radians = -num_radians;
        break;
      case 2:        
        a_rule_object = new Rule('F', "FF");
        ruleset.push(a_rule_object);
      ...
      //Wires
        ...
        case 8:   //experimenting with this one!
          ...
          ruleset.push(a_rule_object);
          lsys = new LSystem("X", ruleset);
          total_iterations = int(p5.random(5,6));
          num_radians = 15;
          if(p5.random(0,100)<50)
            num_radians = num_radians*-1;
          break;
        ...
        case 9:        
          ...
          ruleset.push(a_rule_object);
          lsys = new LSystem("X", ruleset);
          total_iterations = int(p5.random(5,7));
          num_radians = 10;  
          if(p5.random(0,100)<50)
            num_radians = num_radians*-1;
            break; 
    ...
    let max_height = int((6*dim.height)/250);
    let line_height_divisor =int(((max_seeds-seed_index)*max_height)/max_seeds);
   
    //setting the starting x point for the plant and the length of its initial stem
    let turtle = new Turtle(lsys.getSentence(), dim.height/line_height_divisor, p5.radians(num_radians),seed_index, colors, dim);

  //create a fractal object that holds all of this info
  a_fractalObject = new fractalObject(ruleset, lsys, turtle, total_iterations, num_radians, y_point);
 ...
 */
 function  drawTripTychFrames(w, h){
    ...
    //strokeWeight(9);
    //scale strokeWeight
    p5.strokeWeight(int((9/750)*w));
    let curvature = int((30/750)*w);
...
class Turtle {
  constructor( s,  l,  t, _seed_index, colors, dim) {
    ...
    this.thickness = .004*dim.width;//3
  } 

  render() {
    let this_color = this.color;
